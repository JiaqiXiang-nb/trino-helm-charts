...
class JdbcApiTrino(JdbcApi):
  ...
  def __init__(self, user, interpreter=None):
    ...
    if impersonation_property and impersonation_user and self.options['url']:
      self.options['url'] += "&{}={}".format(impersonation_property, impersonation_user)
      del self.options['impersonation_property']
    JdbcApi.__init__(self, user, interpreter=interpreter)

  def _createAssist(self, db):
    return TrinoAssist(db)

class TrinoAssist(Assist):
